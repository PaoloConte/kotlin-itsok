...
inline fun <T> Ok(value: T): Ok<T> = when (value) {
    is Unit -> Ok as Ok<T>
    is ItsOk<*> -> value as Ok<T>
    else -> BaseOk(value)
}
 ...
 */
@Suppress("UNCHECKED_CAST", "NOTHING_TO_INLINE")
inline fun <E> Error(error: E): Error<E> = when (error) {
    is ItsError<*> -> error as Error<E>
    else -> BaseError(error)
}
